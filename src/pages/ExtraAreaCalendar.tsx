...
const getBranchColor = (branch: string): string => {
  ...
  if (lowerBranch.includes('mhb')) return 'bg-orange-500 text-white';
  if (lowerBranch.includes('mlp')) return 'bg-purple-500 text-white';
  if (lowerBranch.includes('msh')) return 'bg-emerald-500 text-white';
  if (lowerBranch.includes('mum')) return 'bg-blue-500 text-white';
  if (lowerBranch.includes('mqc')) return 'bg-pink-500 text-white';
...
const ExtraAreaCalendarPage: React.FC = () => {
  const navigate = useNavigate();
  const { entries, loading } = useExtraAreaStore();
      ...
      <div className="gradient-header px-4 lg:px-6 py-6 relative z-10">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white">Extra Area Calendar</h1>
              <p className="text-white/70 text-sm">View and manage extra area reports by date</p>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1 bg-card rounded-xl border border-border/50 shadow-sm overflow-hidden">
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-border/50">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" className="gap-2" onClick={() => navigate('/extra-area')}>
                        ...
                        {dayEntries.slice(0, 3).map((entry) => (
                          <div
                            ...
                            className={cn(
                              ...
                              getBranchColor(entry.branch)
                            )}
                            title={`${entry.category} - ${entry.locationArea}`}
                          >
                            {entry.category} {entry.locationArea.substring(0, 12)}
                            {entry.locationArea.length > 12 && '...'}
                          </div>
                        ))}
                        {dayEntries.length > 3 && (
...
export default ExtraAreaCalendarPage;
